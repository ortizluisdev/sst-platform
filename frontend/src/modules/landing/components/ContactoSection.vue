  ...
  <section id="contacto" class="relative overflow-hidden bg-cream px-6 py-[120px] pb-[130px] sm:px-8 md:px-16">
    <div class="relative z-[2] mx-auto max-w-[720px]">
      <div v-reveal class="mx-auto mb-12 max-w-[560px] text-center">
        ...
        <i18n-t keypath="contacto.title" tag="h2" class="font-serif text-[clamp(28px,3.6vw,40px)] font-semibold leading-[1.28] text-navy-900">
          <template #accent><em class="font-medium italic text-sky-400">{{ t('contacto.titleAccent') }}</em></template>
        </i18n-t>
      ...
      <form v-if="status !== 'success'" class="grid gap-5 rounded-lg border border-line-strong bg-white p-7 shadow-[0_24px_60px_rgba(11,26,51,0.06)] sm:p-10" novalidate @submit.prevent="submit">
        <div class="grid gap-5 sm:grid-cols-2">
          <div>
            <label for="nombre" class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-navy-700">{{ t('contacto.form.nombre') }} *</label>
            <input id="nombre" v-model="nombre" v-bind="nombreAttrs" type="text" :class="inputClasses" :placeholder="t('contacto.form.nombrePlaceholder')" />
            <p v-if="errors.nombre" class="mt-1.5 text-xs text-red-600">{{ errors.nombre }}</p>
          </div>
          <div>
            <label for="correo" class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-navy-700">{{ t('contacto.form.correo') }} *</label>
            <input id="correo" v-model="correo" v-bind="correoAttrs" type="email" :class="inputClasses" :placeholder="t('contacto.form.correoPlaceholder')" />
        ...
        <div class="grid gap-5 sm:grid-cols-2">
          <div>
            <label for="telefono" class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-navy-700">{{ t('contacto.form.telefono') }} *</label>
            <input id="telefono" v-model="telefono" v-bind="telefonoAttrs" type="tel" :class="inputClasses" :placeholder="t('contacto.form.telefonoPlaceholder')" />
            <p v-if="errors.telefono" class="mt-1.5 text-xs text-red-600">{{ errors.telefono }}</p>
          </div>
          <div>
            <label for="empresa" class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-navy-700">{{ t('contacto.form.empresa') }}</label>
            <input id="empresa" v-model="empresa" v-bind="empresaAttrs" type="text" :class="inputClasses" :placeholder="t('contacto.form.empresaPlaceholder')" />
          </div>
        </div>

        <div>
          <label for="mensaje" class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-navy-700">{{ t('contacto.form.mensaje') }} *</label>
          <textarea
            id="mensaje"
            v-model="mensaje"
            v-bind="mensajeAttrs"
