...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-navy-900/50 px-4" @click.self="emit('cancel')">
    <div class="w-full max-w-md rounded-md bg-white p-5 shadow-xl">
      ...
      <label for="suspend-reason" class="mb-1.5 mt-4 block text-xs font-semibold uppercase tracking-wide text-navy-700">
      ...
      />
      <p v-if="touched && reason.trim().length < MIN_LENGTH" id="suspend-reason-error" class="mt-1.5 text-xs text-red-600">
