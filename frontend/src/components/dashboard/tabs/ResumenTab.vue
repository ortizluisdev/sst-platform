...
<template>
  <div class="grid gap-6">
    <div class="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-line-strong bg-white px-5 py-4">
      <div>
        <h1 class="font-serif text-xl font-semibold text-navy-900">{{ t('dashboard.resumen.titlePrefix') }}{{ dashboard.service.nombre }}</h1>
        <p class="text-xs text-navy-700 opacity-70">{{ t('dashboard.resumen.lastUpdatePrefix') }}{{ lastUpdatedLabel }}</p>
      </div>
    ...
    <div v-if="dashboard.categories.length === 0" class="rounded-lg border border-dashed border-line-strong bg-white p-10 text-center text-sm text-navy-700">
