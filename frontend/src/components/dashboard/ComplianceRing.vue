  ...
  <div class="rounded-lg border border-line-strong bg-white p-4 sm:p-5">
    ...
    <div class="relative mx-auto h-[150px] w-[150px] sm:h-[170px] sm:w-[170px]">
      ...
      <div class="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
        <span v-if="hasData" class="font-serif text-4xl font-semibold text-navy-900">{{ roundDisplay(compliance.pct) }}%</span>
        <span v-else class="font-serif text-lg font-semibold text-navy-700 opacity-70">{{ t('dashboard.complianceRing.noData') }}</span>
        <span v-if="hasData" class="mt-0.5 text-[11px] font-medium uppercase tracking-wide text-navy-700/70">{{
          t('dashboard.complianceRing.centerLabel')
        }}</span>
      </div>
    </div>
    ...
    <div v-if="hasData" class="mt-4 grid grid-cols-3 gap-2 text-center text-xs">
      <div class="rounded-lg border border-emerald-100 bg-emerald-50 px-1.5 py-2">
        ...
        <p class="mt-0.5 text-emerald-700/70">{{ t('dashboard.semaphore.cumple') }}</p>
      </div>
      <div class="rounded-lg border border-amber-100 bg-amber-50 px-1.5 py-2">
        ...
        <p class="mt-0.5 text-amber-700/70">{{ t('dashboard.semaphore.alerta') }}</p>
      </div>
      <div class="rounded-lg border border-red-100 bg-red-50 px-1.5 py-2">
        ...
        <p class="mt-0.5 text-red-700/70">{{ t('dashboard.semaphore.critico') }}</p>
      </div>
    </div>
  </div>
</template>
