...
<template>
  <section class="relative flex min-h-[calc(100vh-1px)] flex-col overflow-hidden bg-cream px-6 py-16 sm:px-8">
    <!-- Mismo fondo animado que ya usa la landing (HeroSection.vue,
    ContactoSection.vue, etc.) — sin props: los valores por defecto del
    composable ya están afinados ahí, no hace falta repetirlos acá. -->
    ...
    y todavía no quiere continuar. z-[2]: por encima del canvas del fondo
    animado, igual que el resto del contenido de esta pantalla. -->
    <router-link
      :to="`/${locale}/`"
      class="group fixed left-5 top-5 z-[2] inline-flex items-center gap-2 text-sm font-medium text-navy-700 transition-colors hover:text-sky-500 sm:left-8 sm:top-8"
    >
      <svg
    ...
    z-[2]: por encima del canvas del fondo animado. -->
    ...
    <footer class="relative z-[2] mt-10 flex flex-col items-center gap-3 print:hidden">
